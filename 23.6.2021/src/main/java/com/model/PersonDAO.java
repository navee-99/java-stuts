...
public class PersonDAO {
	...
	public static boolean insert(Person p) {
		try {
			...
			PreparedStatement preparedStatement=connection.prepareStatement("insert into sampe values(?,?,?)");
...
try      {
		    ...
		    ResultSet rs=connection.createStatement().executeQuery("select *from sampe");
		    while(rs.next()) {
		    	Person p=new Person();
